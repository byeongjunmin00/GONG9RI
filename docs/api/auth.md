...
- 에러:
  ...
  | `ACCOUNT_WITHDRAWN` | 403 | 본인이 탈퇴한 계정(2026-08-21 추가) — 정지와 별개 컬럼(`withdrawnAt`)이다. 정지는 관리자가 풀 수 있지만 탈퇴는 되돌리지 않는다 |
...
> **인증이 필요 없다(멱등).** 이미 로그아웃됐거나 세션이 만료된 상태에서 호출해도 `204`를 반환한다.
> 인증을 요구하던 시절엔, 페이지를 오래 열어둬 세션이 만료된 뒤 로그아웃 버튼을 누르면 401이 나면서
> 화면상 아무 일도 일어나지 않는 버그가 있었다(2026-08-20 수정 —
> `docs/dev/auth/me/changes/003-logout-idempotent.md`).

- 요청 body: 없음 — 세션이 있으면 무효화(`HttpSession.invalidate()`)하고, 없으면 그냥 넘어간다. 어느 쪽이든 `JSESSIONID` 쿠키를 만료시킨다.

- 응답: `204 No Content` (세션 유무와 무관)

- 에러: 없음 — 로그인 상태가 아니어도 성공으로 처리한다.
...
## DELETE /api/member — 회원 탈퇴 (2026-08-21 추가)

`docs/dev/member/withdraw/design.md`.

**행을 지우지 않는다.** 이 회원을 참조하는 테이블이 12개이고 결제·공구팀 참여는 남의 화면에도 영향을 준다 — 지우면 판매자 정산 합계가 틀어지고 같은 팀에 있던 다른 사람들 화면에서 인원이 어긋난다. 대신 `withdrawnAt`을 찍어 로그인을 막고, 이름을 `탈퇴한 회원`으로 덮어쓴다.

- 요청 body: `{ "password": "..." }`
  - 비밀번호는 **본인 확인용**이다. 되돌릴 수 없는 동작이라 로그인 상태만으로는 부족하다고 봤다.
  - 카카오 계정은 비밀번호가 없으므로(가입 시 랜덤 값으로 채움) 이 검사를 건너뛴다. 판정은 서버가 한다.
- 응답: `200 OK` → `data: null`. **응답과 함께 세션이 무효화된다** — 로그인 게이트는 "다시 로그인할 때"만 막으므로, 세션을 안 끊으면 그 브라우저는 탈퇴한 계정으로 계속 돌아다닌다.
- 부수 효과:
  - `profileImageUrl`은 `null`이 되고 파일은 **커밋 후에** 지운다(교체·삭제와 같은 이유).
  - `kakaoId`를 비운다 — 같은 카카오 계정으로 새로 가입할 수 있게 한다.
  - `username`은 남긴다 — UNIQUE 제약이 있고, 같은 아이디로 재가입해 남의 기록을 이어받는 걸 막는다.
  - **판매자면 그 사람 상품을 전부 숨긴다.** 안 그러면 배송할 사람이 없는 상품이 계속 팔린다. 지우지 않는 건 기존 주문 내역·정산에 상품 정보가 그대로 필요해서다.
- 에러:
  | 코드 | HTTP | 설명 |
  |------|------|------|
  | `LOGIN_FAILED` | 401 | 비밀번호 불일치(일반 계정만 해당) |
  | `FORBIDDEN` | 403 | 관리자 계정 — 마지막 관리자가 나가면 아무도 관리자 화면에 들어갈 수 없다 |
  | `ACCOUNT_WITHDRAWN` | 403 | 이미 탈퇴한 계정 |
  | `UNAUTHORIZED` | 401 | 미인증 |
