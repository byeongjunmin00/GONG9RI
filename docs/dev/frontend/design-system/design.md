...
- **헤더 로그인 상태 스코프 경계**: 현재 "현재 로그인 사용자 조회" 엔드포인트가 없어(`docs/api/auth.md`), 헤더는 비로그인 상태 기준 고정 마크업(로그인/회원가입 링크 노출)이다. 로그인 여부에 따른 실제 전환(세션 연동)은 범위 밖 — 로그인 관련 개별 페이지 작업에서 다룬다.
