...
- **헤더 로그인 상태 연동됨 · 로그인 복귀 지원됨(이후 추가됨)**: 이 작업 시점엔 헤더가 비로그인 고정 마크업이었고 로그인 복귀 경로도 없었다. 이후 각각 `frontend/header-auth`(헤더 로그인 상태 반영, `docs/dev/frontend/header-auth/design.md`)와 `feat(frontend): 로그인 후 원래 페이지로 복귀` 커밋(`checkout.js`의 안내 링크에 `?redirect={현재 경로}` 추가, `login.js`가 이를 지원)으로 해소됐다 — 401을 만나 로그인 페이지로 이동해도 로그인 성공 시 checkout으로 복귀한다.
