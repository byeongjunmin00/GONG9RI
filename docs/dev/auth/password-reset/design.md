...
- **실제 Gmail SMTP로 발송 검증 완료(2026-08-12)**: 실제 발신 계정으로 재설정 요청 → 실제 이메일 수신 확인 → 발급된 실제 토큰으로 재설정 실행 → 이전 비밀번호로 로그인 시도 시 `LOGIN_FAILED`(401)로 막히고 새 비밀번호로는 로그인 성공(200)하는 것까지 전체 플로우를 실제로 완주함. 검증 후 실제 DB의 테스트 계정과 Redis 키 전부 정리함.
