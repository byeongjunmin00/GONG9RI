...
- **헤더 티커(`#header-search-trends-ticker`)는 오버레이를 여닫는 토글이다**(2026-08-20 수정). 티커는
  오버레이를 여는 트리거이면서 동시에 오버레이 *바깥*(`.site-header__search` 안쪽)에 있다. 그래서
  "바깥을 클릭하면 닫는다" 핸들러가 티커 클릭을 무시하는데, 티커에 열기만 걸어두면 **열린 상태에서
  눌러도 아무 일이 안 일어나는 죽은 영역**이 된다(사용자가 "안 닫힌다"고 리포트). 여는 트리거를 다시
  누르면 닫히는 게 자연스러워 `togglePanel()`로 처리한다.
- 티커는 2.2초마다 페이드아웃 → 검색어 교체 → 페이드인으로 순환한다(`.is-swapping`, `opacity: 0`).
  오버레이가 열려 있어도 계속 순환한다 — 이걸 사용자가 "깜빡인다"로 인지한 적이 있는데, 오버레이
  안에 전체 순위가 이미 보이는 상태라 헷갈릴 수 있는 지점으로 남겨둔다(현재는 의도된 동작).
