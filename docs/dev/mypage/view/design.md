...
- **판매자 수익 집계** (`docs/db/seller_revenue_summary.md`, 2026-08-06 upsert 전환, `docs/dev/ongoing/seller-revenue-summary-upsert-fix.md`): `mypage/seller-revenue`(`SellerMypageService.revenue()`)는 캐싱하지 않고 `seller_revenue_summary` 테이블을 **순수 조회**만 한다 — 조회 시점에 행을 만드는 쓰기는 없다.
