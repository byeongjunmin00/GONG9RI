...
### 계층형(레이어드) vs 헥사고날 — 레이어드를 선택한 이유

- **팀 규모(2인)와 도메인 복잡도**: 헥사고날(포트/어댑터)이 주는 이점은 "도메인 로직을 인프라(DB/외부API) 세부사항으로부터 완전히 격리"하는 것인데, 이 프로젝트는 도메인 로직 자체가 그렇게 복잡하지 않고(공동구매 정산·상태전이 몇 가지가 핵심) 인프라 교체 가능성(DB를 다른 걸로 바꾼다든지)도 사실상 없다 — 이런 상황에서 포트/어댑터 인터페이스 계층을 추가하면 파일 수·간접 호출 단계만 늘고 실질적 이득은 적다.
- **팀 전체가 이미 익숙한 패턴**: 스프링 부트 학습 과정에서 표준으로 다루는 게 레이어드라, 2인 팀이 컨벤션을 맞추고 코드리뷰 부담을 낮추는 데 유리하다.
- **빠른 반복 우선**: MVP를 빠르게 완성하고 이후 부하테스트·캐싱·AI 기능 등 "고도화"에 시간을 더 쓰는 게 이 프로젝트의 우선순위였다(발제의 6주 타임라인 중 실제로는 MVP를 1~2주 안에 끝내고 나머지를 고도화·AI에 씀) — 레이어드가 이 페이스에 더 잘 맞는다.
- 대신 계층 간 책임을 위 표처럼 엄격하게 지켜서(컨트롤러 비즈니스 로직 금지, 엔티티 직접 노출 금지 등), 헥사고날 없이도 관심사 분리 자체는 확실히 지킨다.
